...
ALTER PROC [dbo].[EXCEL_proc_load_w_spp_market_price_f] @p_batch_id [bigint] AS 
BEGIN
    DECLARE	@tgt_TableName nvarchar(200) = N'dbo.W_EXCEL_SPP_MARKET_PRICE_F',
			@sql nvarchar(max),
    ...
    BEGIN TRY
		...
		LEFT JOIN [dbo].[W_SAP_PLANT_EXTENDED_D] PL_X ON 1=1
			...
			AND (F.PLANT = PL_X.PLANT OR F.PLANT_NAME = PL_X.PLANT_NAME_2)
		WHERE 1=1
        ...
        FROM (
            SELECT *
            FROM W_EXCEL_SPP_MARKET_PRICE_F
        ) M
        WHERE 1=1
