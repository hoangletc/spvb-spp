...
GO
ALTER PROC [dbo].[EXCEL_proc_load_w_spp_purchase_target_f] @p_batch_id [bigint] AS 
BEGIN
    DECLARE	@tgt_TableName nvarchar(200) = N'dbo.W_EXCEL_SPP_PURCHASE_TARGET_F',
			@sql nvarchar(max),
    ...
    BEGIN TRY
        ...
        IF OBJECT_ID(N'tempdb..#W_EXCEL_SPP_PURCHASE_TARGET_tmp') IS NOT NULL 
        BEGIN
            PRINT N'DELETE temporary table #W_EXCEL_SPP_PURCHASE_TARGET_tmp'
            DROP Table #W_EXCEL_SPP_PURCHASE_TARGET_tmp
        END;
		...
		INTO #W_EXCEL_SPP_PURCHASE_TARGET_tmp
		FROM FND.W_EXCEL_SPP_PURCHASE_TARGET_F F
		...
		UPDATE #W_EXCEL_SPP_PURCHASE_TARGET_tmp
		SET W_UPDATE_FLG = 'Y'
		FROM #W_EXCEL_SPP_PURCHASE_TARGET_tmp tg
		INNER JOIN [dbo].[W_EXCEL_SPP_PURCHASE_TARGET_F] sc 
		ON sc.W_INTEGRATION_ID = tg.W_INTEGRATION_ID

		-- 3.2. Start updating
		PRINT '3.2. Start updating'

		UPDATE  [dbo].[W_EXCEL_SPP_PURCHASE_TARGET_F]
		SET 
			[PLANT_WID] = src.[PLANT_WID]
			, [DATE_WID] = src.[DATE_WID]

			, [PLANT] = src.[PLANT]
			, [PURCHASE_TARGET] = src.[PURCHASE_TARGET]
			, [BALANCE_TARGET] = src.[BALANCE_TARGET]
			, [YEAR] = src.[YEAR]
		...
		FROM [dbo].[W_EXCEL_SPP_PURCHASE_TARGET_F] tgt
		INNER JOIN #W_EXCEL_SPP_PURCHASE_TARGET_tmp src ON src.W_INTEGRATION_ID = tgt.W_INTEGRATION_ID
		...
		INSERT INTO [dbo].[W_EXCEL_SPP_PURCHASE_TARGET_F](
			[PLANT_WID]
			, [DATE_WID]
		...
		SELECT
			...
			, W_BATCH_ID
			, W_INTEGRATION_ID
		FROM #W_EXCEL_SPP_PURCHASE_TARGET_tmp
        ...
        FROM (
            SELECT *
            FROM W_EXCEL_SPP_PURCHASE_TARGET_F
        ) M
        WHERE 1=1
            AND W_BATCH_ID = @p_batch_id
            AND W_DELETE_FLG = 'N'

		SET @src_rownum = ( SELECT COUNT(1) FROM #W_EXCEL_SPP_PURCHASE_TARGET_tmp );
		SET @tgt_rownum = ( 
            SELECT 
