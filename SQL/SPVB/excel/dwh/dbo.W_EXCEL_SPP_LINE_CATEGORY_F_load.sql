...
BEGIN
    ...
    BEGIN TRY
        ...
        IF OBJECT_ID(N'tempdb..#W_EXCEL_SPP_LINE_CATEGORY_tmp') IS NOT NULL 
        BEGIN
            PRINT N'DELETE temporary table #W_EXCEL_SPP_LINE_CATEGORY_tmp'
            DROP Table #W_EXCEL_SPP_LINE_CATEGORY_tmp
        END;
		...
		INTO #W_EXCEL_SPP_LINE_CATEGORY_tmp
		FROM FND.W_EXCEL_SPP_LINE_CATEGORY_F
		;
		...
		UPDATE #W_EXCEL_SPP_LINE_CATEGORY_tmp
		SET W_UPDATE_FLG = 'Y'
		FROM #W_EXCEL_SPP_LINE_CATEGORY_tmp tg
		INNER JOIN [dbo].[W_EXCEL_SPP_LINE_CATEGORY_F] sc 
		ON sc.W_INTEGRATION_ID = tg.W_INTEGRATION_ID

		-- 3.2. Start updating
		PRINT '3.2. Start updating'

		UPDATE  [dbo].[W_EXCEL_SPP_LINE_CATEGORY_F]
		SET 
		...
		FROM [dbo].[W_EXCEL_SPP_LINE_CATEGORY_F] tgt
		INNER JOIN #W_EXCEL_SPP_LINE_CATEGORY_tmp src ON src.W_INTEGRATION_ID = tgt.W_INTEGRATION_ID
		...
		INSERT INTO [dbo].[W_EXCEL_SPP_LINE_CATEGORY_F](
			[LINE_ASSET_NUM]
			, [CATEGORY]
			, [FROM_DATE]
			, [TO_DATE]
		...
		SELECT
			...
			, W_BATCH_ID
			, W_INTEGRATION_ID
		FROM #W_EXCEL_SPP_LINE_CATEGORY_tmp
        ...
        FROM (
            SELECT *
            FROM W_EXCEL_SPP_LINE_CATEGORY_F
        ) M
        WHERE 1=1
            AND W_BATCH_ID = @p_batch_id
            AND W_DELETE_FLG = 'N'

		SET @src_rownum = ( SELECT COUNT(1) FROM #W_EXCEL_SPP_LINE_CATEGORY_tmp );
		SET @tgt_rownum = ( 
            SELECT 
