...
ALTER PROC [dbo].[EXCEL_proc_load_fnd_w_spp_line_category_f] @p_batch_id [bigint] AS

BEGIN
	--DECLARE @p_batch_id bigint = 2020111601
	DECLARE	@tgt_TableName nvarchar(200) = N'FND.W_EXCEL_SPP_LINE_CATEGORY_F',
			@sql nvarchar(max),
	...
	BEGIN TRY
		...
		UPDATE FND.W_EXCEL_SPP_LINE_CATEGORY_F SET 
			W_DELETE_FLG = 'Y'
			, W_UPDATE_DT = DATEADD(HH, 7, GETDATE())
			, W_BATCH_ID = @p_batch_id
		WHERE W_DELETE_FLG = 'N'
			AND FILE_PATH IN (SELECT DISTINCT FILE_PATH FROM STG.W_EXCEL_SPP_LINE_CATEGORY_FS /*WHERE W_BATCH_ID = @p_batch_id*/)
	;
	WITH A AS (																	
		SELECT 
			CONVERT(NVARCHAR(10), PLANT)					AS PLANT
			, CONVERT(NVARCHAR(100), LINE_ASSET_NUM)		AS [LINE_ASSET_NUM]
			, CONVERT(NVARCHAR(1000), LINE_DESCRIPTION)		AS [LINE_ASSET_DESCRIPTION]
			, CONVERT(NVARCHAR(100), CATEGORY)				AS [CATEGORY]
			, CONVERT(datetime2, FROM_DATE)					AS [FROM_DATE]
			, CONVERT(datetime2, TO_DATE)					AS [TO_DATE]
			, FILE_PATH

			, CONCAT([LINE_ASSET_NUM], '~', 
					PLANT, '~', [CATEGORY])					AS W_INTEGRATION_ID
		FROM STG.W_EXCEL_SPP_LINE_CATEGORY_FS
	)
	INSERT INTO FND.W_EXCEL_SPP_LINE_CATEGORY_F(
		PLANT
		, LINE_ASSET_NUM
...
/*
		...
		WHERE W_BATCH_ID = @p_batch_id
			AND W_DELETE_FLG = 'N'
*/
		SET @src_rownum = (SELECT COUNT(1) FROM [STG].[W_EXCEL_SPP_LINE_CATEGORY_FS] WHERE W_BATCH_ID = @p_batch_id);
		SET @tgt_rownum = (SELECT COUNT(1) FROM FND.W_EXCEL_SPP_LINE_CATEGORY_F WHERE W_DELETE_FLG = 'N' AND  W_BATCH_ID = @p_batch_id);
