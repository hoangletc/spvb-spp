...
alter PROC [dbo].[EXCEL_proc_load_fnd_w_spp_mapping_sloc_sap_cmms_d] @p_batch_id [bigint] AS

BEGIN
	--DECLARE @p_batch_id bigint = 2020111601
	DECLARE	@tgt_TableName nvarchar(200) = N'FND.W_EXCEL_SPP_MAPPING_SLOC_SAP_CMMS_D',
			@sql nvarchar(max),
	...
	BEGIN TRY		
		INSERT INTO FND.W_EXCEL_SPP_MAPPING_SLOC_SAP_CMMS_D(
			[SAP_PLANT]
			, [SAP_PLANT_DESCRIPTION]
			, [SAP_STO_LOC]
			, [SAP_STO_LOC_DESCRIPTION]

			, [CMMS_PLANT]
			, [CMMS_STO_LOC]
			, [CMMS_STO_LOC_DESCRIPTION]
		...
		select 
			CONVERT(NVARCHAR(10), [SAP Plant (WERKS)])
			, CONVERT(NVARCHAR(1000), [SAP plant description])
			, CONVERT(NVARCHAR(10), [SAP Storage location (LGORT)])
			, CONVERT(NVARCHAR(1000), [SAP Storage location description])
			, CONVERT(NVARCHAR(10), [CMMS Plant (SIDEID)])
			, CONVERT(NVARCHAR(10), [CMMS Storage location (TOSTORELOC/FROMSTORELOC)])
			, CONVERT(NVARCHAR(1000), [CMMS Storage location description])
			...
			, CONCAT([SAP Plant (WERKS)], '~', 
					[SAP Storage location (LGORT)]) 					AS W_INTEGRATION_ID
		...
		SET @src_rownum = (SELECT COUNT(1) FROM STG.W_EXCEL_SLOC_MAPPING_CMMS_VS_SAP_DS);
		SET @tgt_rownum = (SELECT COUNT(1) FROM FND.W_EXCEL_SPP_MAPPING_SLOC_SAP_CMMS_D WHERE W_DELETE_FLG = 'N' AND  W_BATCH_ID = @p_batch_id);
