...
ALTER PROC [dbo].[EXCEL_proc_load_fnd_w_spp_lack_spp_f] @p_batch_id [bigint] AS

BEGIN
	--DECLARE @p_batch_id bigint = 2020111601
	DECLARE	@tgt_TableName nvarchar(200) = N'FND.W_EXCEL_SPP_LACK_SPP_F',
			@sql nvarchar(max),
	...
	BEGIN TRY
		...
		UPDATE FND.W_EXCEL_SPP_LACK_SPP_F SET 
			W_DELETE_FLG = 'Y'
			, W_UPDATE_DT = DATEADD(HH, 7, GETDATE())
			, W_BATCH_ID = @p_batch_id
		WHERE W_DELETE_FLG = 'N'
			AND FILE_PATH IN (SELECT DISTINCT FILE_PATH FROM STG.W_EXCEL_SPP_LACK_SPP_FS /*WHERE W_BATCH_ID = @p_batch_id*/)
	;
		
		INSERT INTO FND.W_EXCEL_SPP_LACK_SPP_F(
			[ITEM_NUM]
			, [DESCRIPTION]
			, [WAREHOUSE]
			, [LACK_SPP]
		...
		select 
			CONVERT(NVARCHAR(15), [ITEMNUM])					AS [ITEM_NUM]
			, CONVERT(NVARCHAR(1000), [DESCRIPTION])			AS [DESCRIPTION]
			, CONVERT(NVARCHAR(10), [WAREHOUSE])				AS [WAREHOUSE]
			, CASE WHEN [LACK OF SPP] = 'Y' THEN 1 ELSE 0 END	AS [LACK_SPP]
			...
			, CONCAT([ITEMNUM], '~', [WAREHOUSE])				AS W_INTEGRATION_ID
		from STG.W_EXCEL_SPP_LACK_SPP_FS
		;
		...
		SET @src_rownum = (SELECT COUNT(1) FROM [STG].[W_EXCEL_SPP_LACK_SPP_FS] WHERE W_BATCH_ID = @p_batch_id);
		SET @tgt_rownum = (SELECT COUNT(1) FROM FND.W_EXCEL_SPP_LACK_SPP_F WHERE W_DELETE_FLG = 'N' AND  W_BATCH_ID = @p_batch_id);
