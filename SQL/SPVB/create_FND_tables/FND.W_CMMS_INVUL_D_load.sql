...
ALTER PROC [dbo].[SAP_proc_load_w_cmms_spp_fnd_invul_d]
    @p_batch_id [bigint]
AS
BEGIN
    DECLARE	@tgt_TableName nvarchar(200) = N'FND.W_CMMS_INVUL_D',
			@sql nvarchar(max),
    ...
    BEGIN TRY
        ...
        SELECT
            ...
            , CONVERT(REAL, [UNITCOST])                                 AS [UNITCOST]
            , CONVERT(NVARCHAR(100), [SPVB_EXTREASONCODE])              AS [SPVB_EXTREASONCODE]
            , CONVERT(NVARCHAR(1000), [SPVB_EXTREASONCODE_DESCRIPTION]) AS [SPVB_EXTREASONCODE_DESCRIPTION]
            , CONVERT(NVARCHAR(100), [SPVB_MUSTRETURN])                 AS [SPVB_MUSTRETURN]
            , CONVERT(NVARCHAR(100), [SPVB_RETURNFROMISSUE])            AS [SPVB_RETURNFROMISSUE]
            , CONVERT(NVARCHAR(100), [SPVB_MUSTRETURN_ORG])             AS [SPVB_MUSTRETURN_ORG]
            , CONVERT(REAL, [RETURNED_QTY])                             AS [RETURNED_QTY]
            , CONVERT(NVARCHAR(100), [REMARK])                          AS [REMARK]
            ...
            , CONVERT(NVARCHAR(100), [INVUSE_NUM])                      AS [INVUSE_NUM]
            , CONVERT(DATETIME2, [ACTUALDATE], 103)                     AS [ACTUALDATE]
            , CONVERT(REAL, [RECEIVED_QTY])                             AS [RECEIVED_QTY]
            , CONVERT(NVARCHAR(100), [ASSET_NUM])                       AS [ASSET_NUM]
            , CONVERT(NVARCHAR(100), [COSTCENTER])                      AS [COSTCENTER]
            , CONVERT(NVARCHAR(100), [FROM_STORELOC])                   AS [FROM_STORELOC]
            , CONVERT(NVARCHAR(100), [REFWO])                           AS [REFWO]
            , CONVERT(REAL, [LINECOST])                                 AS [LINECOST]
            , CONVERT(REAL, [QUANTITY])                                 AS [QUANTITY]
            , CONVERT(NVARCHAR(100), [COSTCENTER_DESCRIPTION])          AS [COSTCENTER_DESCRIPTION]
            , CONVERT(NVARCHAR(100), [INVUSELINE_NUM])                  AS [INVUSELINE_NUM]
            , CONVERT(NVARCHAR(100), [ITEM_NUM])                        AS [ITEM_NUM]
            , CONVERT(NVARCHAR(100), [ITEMSETID])                       AS [ITEMSETID]
            , CONVERT(NVARCHAR(100), [LOCATION])                        AS [LOCATION]
            , CONVERT(NVARCHAR(100), [SPVB_SAPPO])                      AS [SPVB_SAPPO]
            , CONVERT(NVARCHAR(100), [USE_TYPE])                        AS [USE_TYPE]
            , CONVERT(NVARCHAR(100), [ENTER_BY])                        AS [ENTER_BY]
            , CONVERT(NVARCHAR(100), [SPVB_WONUMREF])                   AS [SPVB_WONUMREF]
            , CONVERT(NVARCHAR(100), [SPVB_REASON])                     AS [SPVB_REASON]

            , CONVERT(NVARCHAR(100), [FROM])                            AS [FROM]
            , CONVERT(NVARCHAR(100), [FILE_NAME])                       AS [FILE_NAME]

            , CONVERT(NVARCHAR, CONCAT(INVUSELINE_ID, '~', [FROM]))     AS W_INTEGRATION_ID
            , 'N'                                                       AS W_DELETE_FLG
            , 'N' 											            AS W_UPDATE_FLG
        ...
        INSERT INTO [FND].[W_CMMS_INVUL_D](
            INVUSELINE_ID
