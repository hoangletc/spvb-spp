		...
		TMP_WO AS (
			...
			FROM [FND].[W_SAP_ACDOCA_SPP_F] F
				LEFT JOIN TMP_WO ON 1=1
					AND TMP_WO.WONUM = F.UPLOADING_POINT
				...
				LEFT JOIN [dbo].[W_SAP_COST_CENTER_D] COST_CENTER ON 1=1
					AND COST_CENTER.COST_CENTER = CASE WHEN CONVERT(NVARCHAR(20), RCNTR) = ''
						THEN CONVERT(VARCHAR(50), KOSTL)
						ELSE CONVERT(NVARCHAR(20), RCNTR) END
