...
IF (OBJECT_ID('[dbo].[SAP_proc_load_w_spp_transaction_f]') is not null)
BEGIN
    DROP PROCEDURE [dbo].[SAP_proc_load_w_spp_transaction_f]
END;
GO


CREATE PROC [dbo].[SAP_proc_load_w_spp_transaction_f] @p_batch_id [bigint] AS 
BEGIN
		...
		;WITH MATDOC_EXTENDED AS (
			...
			FROM [FND].[W_SAP_ACDOCA_SPP_F] ACDOCA
				...
				LEFT JOIN [dbo].[W_SAP_COST_CENTER_D] COST_CENTER ON 1=1
					AND COST_CENTER.COST_CENTER = ACDOCA.RCNTR 
