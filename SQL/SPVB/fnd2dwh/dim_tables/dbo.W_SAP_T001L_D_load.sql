...
IF (OBJECT_ID('[dbo].[proc_load_w_sap_t001l_d]') is not null)
BEGIN
    DROP PROCEDURE [dbo].[proc_load_w_sap_t001l_d]
END;
GO

CREATE PROC [dbo].[proc_load_w_sap_t001l_d]
    @p_batch_id [bigint]
AS 
BEGIN
    DECLARE	@tgt_TableName nvarchar(200) = N'dbo.W_SAP_T001L_D',
			@sql nvarchar(max),
    ...
    BEGIN TRY
        ...
        IF OBJECT_ID(N'tempdb..#W_SAP_T001L_D_tmp') IS NOT NULL 
        BEGIN
            PRINT N'DELETE temporary table #W_SAP_T001L_D_tmp'
            DROP Table #W_SAP_T001L_D_tmp
        END;
		...
		SELECT
            ...
            , CONVERT(nvarchar(100), LGORT) AS LGORT
            , CONVERT(nvarchar(100), LGOBE) AS LGOBE
            , CONVERT(nvarchar(100), SPART) AS SPART
            , CONVERT(nvarchar(100), XLONG) AS XLONG
            , CONVERT(nvarchar(100), XBUFX) AS XBUFX
            , CONVERT(nvarchar(100), DISKZ) AS DISKZ
            , CONVERT(nvarchar(100), XBLGO) AS XBLGO
            , CONVERT(nvarchar(100), XRESS) AS XRESS
            , CONVERT(nvarchar(100), XHUPF) AS XHUPF
            , CONVERT(nvarchar(100), PARLG) AS PARLG
            , CONVERT(nvarchar(100), VKORG) AS VKORG
            , CONVERT(nvarchar(100), VTWEG) AS VTWEG
            , CONVERT(nvarchar(100), VSTEL) AS VSTEL
            , CONVERT(nvarchar(100), LIFNR) AS LIFNR
            , CONVERT(nvarchar(100), KUNNR) AS KUNNR
            , CONVERT(nvarchar(100), MESBS) AS MESBS
            , CONVERT(nvarchar(100), MESST) AS MESST
            , CONVERT(nvarchar(100), OIH_LICNO) AS OIH_LICNO
            , CONVERT(nvarchar(100), OIG_ITRFL) AS OIG_ITRFL
            , CONVERT(nvarchar(100), OIB_TNKASSIGN) AS OIB_TNKASSIGN

			, CONVERT([varbinary](100), CONCAT_WS('~', MANDT, WERKS, LGORT)) AS W_INTEGRATION_ID
			, 'N' AS W_DELETE_FLG
        ...
        INTO #W_SAP_T001L_D_tmp
        FROM [FND].[W_SAP_T001L_D]
        ...
        UPDATE #W_SAP_T001L_D_tmp
		SET W_UPDATE_FLG = 'Y'
		FROM #W_SAP_T001L_D_tmp tg
        INNER JOIN [dbo].[W_SAP_T001L_D] sc 
        ON sc.W_INTEGRATION_ID = tg.W_INTEGRATION_ID

        -- 3.2. Start updating
        PRINT '3.2. Start updating'

		UPDATE  [dbo].[W_SAP_T001L_D]
		SET 
            MANDT = src.MANDT
            , WERKS = src.WERKS
            , LGORT = src.LGORT
            ...
            , MESBS = src.MESBS
            , MESST = src.MESST
            , OIH_LICNO = src.OIH_LICNO
            , OIG_ITRFL = src.OIG_ITRFL
            , OIB_TNKASSIGN = src.OIB_TNKASSIGN
        ...
        FROM [dbo].[W_SAP_T001L_D] tgt
        INNER JOIN #W_SAP_T001L_D_tmp src ON src.W_INTEGRATION_ID = tgt.W_INTEGRATION_ID
		...
		SET @src_rownum = ( SELECT COUNT(1) FROM #W_SAP_T001L_D_tmp );
		SET @tgt_rownum = ( 
            SELECT 
