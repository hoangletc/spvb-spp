...
AS
BEGIN
    DECLARE	@tgt_TableName nvarchar(200) = N'dbo.W_CMMS_LOC_D',
			@sql nvarchar(max),
    ...
    BEGIN TRY
        ...
        IF OBJECT_ID(N'tempdb..#W_CMMS_LOC_D_tmp') IS NOT NULL 
        BEGIN
        PRINT N'DELETE temporary table #W_CMMS_LOC_D_tmp'
        DROP Table #W_CMMS_LOC_D_tmp
    END;
        ...
        UPDATE #W_CMMS_LOC_D_tmp
		SET W_UPDATE_FLG = 'Y'
		FROM #W_CMMS_LOC_D_tmp tg
        INNER JOIN [dbo].[W_CMMS_LOC_D] sc
		...
		SET 
            ...
            , [SITE] = src.SITE
            , TYPE_DESCRIPTION = src.TYPE_DESCRIPTION
